...
static struct clkctl_acpu_speed acpu_freq_tbl[] = {
		...
		{ 476160, PLL_1,    2, 0,  153600, 1100, VDD_RAW(1100) },
		{ 599040, PLL_1,    2, 0,  153600, 1100, VDD_RAW(1100) },
		{ 691200, PLL_1,    2, 0,  153600, 1100, VDD_RAW(1100) },
	/* Make sure any freq based from PLL_2 is a multiple of 19200! */
		{ 806400, PLL_2,    3, 0,  192000, 1100, VDD_RAW(1100) },
		{ 902400, PLL_2,    3, 0,  192000, 1150, VDD_RAW(1150) },
		{ 998400, PLL_2,    3, 0,  192000, 1175, VDD_RAW(1175) },
		{ 1094400, PLL_2,   3, 0,  192000, 1200, VDD_RAW(1200) },
		{ 1152000, PLL_2,   3, 0,  192000, 1225, VDD_RAW(1225) },
		{ 1209600, PLL_2,   3, 0,  192000, 1250, VDD_RAW(1250) },
		{ 1305600, PLL_2,   3, 0,  192000, 1275, VDD_RAW(1275) },
		{ 1401600, PLL_2,   3, 0,  192000, 1275, VDD_RAW(1275) },
...
void acpuclk_set_vdd(unsigned acpu_clk_khz, int vdd)
{
	...
	for (i = 0; acpu_freq_tbl[i].acpu_clk_khz; i++)
	{
		...
		{
			if (acpu_clk_khz == 0)
				acpu_freq_tbl[i].vdd_mv = min(max((acpu_freq_tbl[i].vdd_mv + vdd), VISION_MIN_UV_MV), VISION_MAX_UV_MV);
			else if (acpu_freq_tbl[i].acpu_clk_khz == acpu_clk_khz)
				acpu_freq_tbl[i].vdd_mv = min(max(vdd, VISION_MIN_UV_MV), VISION_MAX_UV_MV);
		}
	}
	mutex_unlock(&drv_state.lock);
}
